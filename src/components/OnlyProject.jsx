        ...
        project = <Notification isColor='light'>
            <Query query={ GET_PROJECTS_EXTENDED_INFO } variables={{id: Number(slug)}}>
                {({loading, error, data}) => {
                    ...
                    if (error) return <Content><Trans i18nKey="project_page.load_error" /><Progress max={100}/></Content>;
                    const current = data.GetProjects;

                    if(current.length){
                        let urlTo;
                        if(current[0].url){
                            urlTo = <Button style={{marginTop: 30}} isLink isSize='medium' isColor='info' href={ current[0].url } target="_blank"><Trans i18nKey="project_page.project_site" /></Button>
                        }
